...
function parse_line(ln::AbstractString)
    ...
    CHROM2 = svclass == "BND" ? replace(match(r"CHR2=chr[\d\w_]+", INFO).match, "CHR2=" => "") : CHROM
    ...
    for line in eachline(fs)
        if startswith(line, "#")
            ...
            elseif svclass == "DEL"
                ...
                println(OUT, CHROM, "\t", POS, "\t", POS + 1, "\t", CHROM2, "\t", POS2, "\t", POS2 + 1, "\t", ID, "\t", pesupport, "\t", "-", "\t", "+", "\t", "DUP", "\t", "Delly")
            end
        end
    end
end
...
@main function main(; input::String="none")
    for line in eachline(input)
        prx, fs = split(line, "\t")
        open("format.$(prx).sv.delly.txt", "w+") do io
            parse_delly(fs, io)
        end
    end
end
